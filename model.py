...
def loss_fn(out, target_relation, num_classes):
    ...
    target_relation = target_relation.view(-1)
    return lfn(out_logits, target_relation)
...
class REModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, target_relation=None):
        output = self.bert(input_ids, attention_mask, token_type_ids)
        concat_2_hidden = torch.cat(tuple([output.hidden_states[i] for i in [-1, -2]]),
                                    dim=-1)  # [batch_size, seq_len, hidden*2]
